        ...
        <div class="card content-area">
            <div class="card-content app-wrapper">
                <div class="collection email-collection">
                    @foreach(\App\TaskComments::where('task_id',$taskInfo->id)->orderBy('id','desc')->get() as $key => $value)
                        ...
                        <div class="email-brief-info collection-item animate fadeUp delay-1">
                            ...
                            <div class="list-right">
                                <div class="favorite">
                                    <a href="{{ route('comments.like',['id'=>$value->id]) }}" class="tooltipped" data-position="bottom" data-tooltip="Beğeni ({{ $value->likeCount }})">
                                ...
                                @if(\Illuminate\Support\Facades\Auth::id()==$value->user_id)
                                    ...
                                    <div class="favorite">
                                        <a href="{{ route('comments.destroy',['id'=>$value->id]) }}" class="tooltipped" data-position="bottom" data-tooltip="Sil"><i class="material-icons red-text">delete</i></a>
                                    </div>
                                @endif
                            </div>
                        </div>
                        @if(\Illuminate\Support\Facades\Auth::id()==$value->user_id)
                            <form class="row m-0" id="commentedit{{ $value->id }}" method="POST" action="{{ route('comments.update',['id'=>$value->id]) }}" style="display: none">
                                @csrf
                                <input type="hidden" name="task_id" value="{{ $taskInfo->id }}">
